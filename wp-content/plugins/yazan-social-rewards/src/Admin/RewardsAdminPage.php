...
/**
 * "Yazan Rewards → Rewards" — a no-code rewards manager. Create/edit unlimited
 * rewards of any type (credit, coupons, free shipping/product, VIP & ring
 * services, exclusive products) with points cost, availability window, expiration,
 * stock and per-user usage limit. Driven by admin-rewards.js against the
 * yazan-rewards/v1/admin/rewards API.
 */
final class RewardsAdminPage implements Hookable {

	/** Submenu slug. */
	public const SLUG = 'yazan-rewards-rewards';
	...
	public function register_menu(): void {
		...
		if ( isset( $admin_page_hooks[ self::PARENT ] ) ) {
			add_submenu_page( self::PARENT, __( 'Rewards', 'yazan-rewards' ), __( 'Rewards', 'yazan-rewards' ), Capabilities::MANAGE, self::SLUG, array( $this, 'render' ) );
		} else {
	...
	public function render(): void {
		...
		echo '<h1>' . esc_html__( 'Rewards', 'yazan-rewards' ) . '</h1>';
		echo '<p class="description">' . esc_html__( 'Create and manage the rewards customers redeem points for. No coding required.', 'yazan-rewards' ) . '</p>';
		echo '<div id="yzrw-rewards-app" class="yzrw-admin-app" aria-live="polite"></div>';
		echo '</div>';
	}
	...
	public function enqueue( $hook_suffix ): void {
		...
		wp_enqueue_script( 'yazan-admin-rewards', $this->assets->url( 'assets/js/admin-rewards.js' ), array(), $this->assets->version( 'assets/js/admin-rewards.js' ), true );
		wp_localize_script(
			'yazan-admin-rewards',
			'YazanRewardsAdmin',
			array(
				'restUrl' => esc_url_raw( rest_url( 'yazan-rewards/v1/admin/rewards' ) ),
